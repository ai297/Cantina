...
namespace Cantina.Services
{
    ...
    public class ChatTimerService : BackgroundService
    {
        ...
        public ChatTimerService(OnlineUsersService onlineService, ILogger<ChatTimerService> logger, MessageService messageService, IOptions<ServicesOptions> options)
        {
        ...
        {
            var archiveSavingInterval = TimeSpan.FromMinutes(_options.Value.ArchiveSavingInterval);
            ...
            _logger.LogInformation($"Messages will be archived with an interval of {_options.Value.ArchiveSavingInterval} min.");
            
            var usersUpdateInterval = TimeSpan.FromMinutes(_options.Value.OnlineUsersCheckItnerval);
            onlineUsersTimer = new Timer(async (object state) =>
            {
                await _onlineUsers.CheckUsersStatus(usersUpdateInterval);
            },
            null, usersUpdateInterval, usersUpdateInterval);
            _logger.LogInformation($"Online users will be checked every {_options.Value.OnlineUsersCheckItnerval} min.");
